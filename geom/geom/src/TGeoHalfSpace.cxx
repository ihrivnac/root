...
Double_t TGeoHalfSpace::DistFromInside(const Double_t *point, const Double_t *dir, Int_t iact, Double_t step,
                                       Double_t *safe) const
{
   Double_t r[3];
   r[0] = fP[0] - point[0];
   ...
   if (TMath::Abs(ddotn) < TGeoShape::Tolerance())
      return snxt;
   snxt = rdotn / ddotn;
   if (snxt < 0)
      return TGeoShape::Big();
...
Double_t TGeoHalfSpace::DistFromOutside(const Double_t *point, const Double_t *dir, Int_t iact, Double_t step,
                                        Double_t *safe) const
{
   Double_t r[3];
   r[0] = fP[0] - point[0];
...
void TGeoHalfSpace::SavePrimitive(std::ostream &out, Option_t * /*option*/ /*= ""*/)
{
   ...
   out << "   TGeoShape *" << GetPointerName() << " = new TGeoHalfSpace(\"" << GetName() << "\", point,norm);"
       << std::endl;
...
void TGeoHalfSpace::DistFromInside_v(const Double_t *points, const Double_t *dirs, Double_t *dists, Int_t vecsize,
                                     Double_t *step) const
{
   for (Int_t i = 0; i < vecsize; i++)
...
void TGeoHalfSpace::DistFromOutside_v(const Double_t *points, const Double_t *dirs, Double_t *dists, Int_t vecsize,
                                      Double_t *step) const
{
   for (Int_t i = 0; i < vecsize; i++)
