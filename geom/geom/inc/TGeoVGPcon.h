...
#ifndef ROOT_TGeoVGPcon
#define ROOT_TGeoVGPcon
...
namespace vecgeom {
  ...
  class GenericUnplacedPolycone : public VUnplacedVolume {
...
class TGeoVGPcon final : public TGeoVGAdapter<vecgeom::GenericUnplacedPolycone> {
  using Shape_t = vecgeom::GenericUnplacedPolycone;
  using Base_t  = TGeoVGAdapter<vecgeom::GenericUnplacedPolycone>;
private:
...
protected:
   ...
   Double_t fCdphi = 0;       //! Cosine of dphi
    ...
    vecgeom::GenericUnplacedPolycone* fRealVolume = nullptr;

   // methods
   TGeoVGPcon(const TGeoVGPcon &) = delete;
   TGeoVGPcon &operator=(const TGeoVGPcon &) = delete;
...
public:
   // constructors
   TGeoVGPcon();
   TGeoVGPcon(const char *name);
   TGeoVGPcon(Double_t phi, Double_t dphi, Int_t nz);
   TGeoVGPcon(const char *name, Double_t phi, Double_t dphi, Int_t nz);
   TGeoVGPcon(Double_t *params);
   // destructor
   ~TGeoVGPcon() override;
   ...
   //     return fRealVolume->Contains_v(points, inside, vecsize);
   // }
   // void ComputeNormal_v(const Double_t *points, const Double_t *dirs, Double_t *norms, Int_t vecsize) override {
   //    return fRealVolume->ComputeNormal_v(pointts, dirs, norms, vecsize);
   // }
   // void DistFromInside_v(const Double_t *points, const Double_t *dirs, Double_t *dists, Int_t vecsize,
   //                       Double_t *step) const override {
   //    return fRealVolume->DistFromInside_v(points, dirs, dists, vecsize, step);      
   // }
   // void DistFromOutside_v(const Double_t *points, const Double_t *dirs, Double_t *dists, Int_t vecsize,
   //                        Double_t *step) const override {
   //    return fRealVolume->DistFromOutside_v(points, dirs, dists, vecsize, step);            
   // }
   // void Safety_v(const Double_t *points, const Bool_t *inside, Double_t *safe, Int_t vecsize) const override {
   //    return fRealVolume->Safety_v(points, inside, safe, vecsize);            
   // }
   ...
   DistanceToIn(U3Vector const& position, U3Vector const& direction,
     ...
     std::cout << "Calling TGeoVGPcon DistanceToIn" << std::endl;
   ...
   // ClassDefOverride(TGeoVGPcon, 1) // polycone class
};
